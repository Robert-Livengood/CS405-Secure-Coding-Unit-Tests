...
// Test to verify the addition of a specific integer is saved correctly in collection
TEST_F(CollectionTest, VerifyAdditionOfSpecficEntry)
{
    // add integer "1" to collection
    collection->push_back(1);

    // get value located at index 0
    int value = collection->at(0);

    // verify that the collection size is 1
    EXPECT_TRUE(collection->size() == 1);
    
    // verify that value is equal to 1
    ASSERT_EQ(value, 1);
}

// Test to verify that collection items within a range are eraseded
TEST_F(CollectionTest, RemoveRangeOfVariables)
{
    // add sequential integers 1-5 to the collection
    for (int i = 0; i < 5; i++)
    {
        collection->push_back(i + 1);
        EXPECT_EQ(collection->at(i), i + 1);
    }

    // verify collection size
    EXPECT_EQ(collection->size(), 5);

    // remove value at indices 2-4 leaving only 1 and 5 in the collection
    collection->erase(collection->begin() + 1, collection->end() - 1);
    EXPECT_EQ(collection->size(), 2);

    // verify that the remaining values are 1 and 5
    ASSERT_EQ(collection->at(0), 1);
    ASSERT_EQ(collection->at(1), 5);
}

// Test to demonstrate Binary Search
TEST_F(CollectionTest, BinarySearchForIndexOfValue)
{
    ...
    // define search space and target value
    int lowIndex = 0;
    int highIndex = 4;
    int targetValue = 3;
    int targetIndex = -1;

    // sort collection into ascending order
    std::sort(collection->begin(), collection->end());

    // loop while search executes
    while (lowIndex <= highIndex)
    {
        // find midpoint in search space and compare to target
        int midIndex = (lowIndex + highIndex) / 2;

        if (collection->at(midIndex) == targetValue)
        {
            targetIndex = midIndex;
            break;
        }
        else if (targetValue < collection->at(midIndex))
        {
    ...
    // Verify that the found index is the correct one
    EXPECT_NE(targetIndex, -1);
    EXPECT_EQ(collection->at(targetIndex), targetValue);
